    ...
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        ...
        <div class="relative bg-white/40 backdrop-blur-xl rounded-lg shadow-sm dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
                <div>
                    <h3 class="font-kiwi text-xl font-semibold  text-white dark:text-white">
                            {{ ScheduleEvent?.title }}
                    </h3>
                    <div class="flex items-center">
                        <MapPinIcon class="h-6 w-6 text-white" />
                        <p class="text-white font-kiwi">{{ ScheduleEvent?.place }}</p>
                    </div>
                    <div class="flex items-center">
                        <ClockIcon class="h-6 w-6 text-white" />
                        <p class="font-kiwi text-white">{{ ScheduleEvent?.time }}</p>
                    </div>
                </div>
                <button type="button" data-modal-hide="schedule-modle" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-start items-center dark:hover:bg-gray-600 dark:hover:text-white">
...
import { MapPinIcon,MapIcon, ClockIcon } from '@heroicons/vue/24/outline'
