<template>
    <div id="schedule" class="w-4/5 mt-24">
        <p  class="text-2xl font-semibold font-kiwi mb-10">スケジュール</p>        
        <ol class="relative border-s border-gray-200 dark:border-gray-700">                  
            <li  v-for="(events, hour) in schedules" :key="hour" class="ms-4 font-kiwi">
                <div class="absolute w-3 h-3 bg-gray-200/30 rounded-full mt-1.5 -start-1.5 border border-white dark:border-gray-900 dark:bg-gray-700"></div>
                <time class="mb-1 text-sm font-normal leading-none text-gray-900 dark:text-gray-500">{{ hour }}</time>
                <!-- <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Application UI code in Tailwind CSS</h3>
                <p class="mb-4 text-base font-normal text-gray-500 dark:text-gray-400">Get access to over 20+ pages including a dashboard layout, charts, kanban board, calendar, and pre-order E-commerce & Marketing pages.</p> -->
                <div class="overflow-auto flex flex-nowrap items-baselin py-4">
                    <button @click="openModle(event)"  data-modal-target="schedule-modle" data-modal-toggle="schedule-modle" v-for="(event, eventId) in events" :key="eventId" class="mb-3 backdrop-blur-sm shrink-0 inline-flex items-center px-4 py-2 mx-3 text-sm font-medium text-gray-900 bg-white/30 border border-gray-200 rounded-lg hover:bg-gray-100/30 hover:text-blue-700 focus:z-10 focus:ring-4 focus:outline-none focus:ring-gray-100 focus:text-blue-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-700">{{ event.title }}<svg class="w-3 h-3 ms-2 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
